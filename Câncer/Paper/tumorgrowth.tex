...
\begin{algorithm}[h]
  \small
  \DontPrintSemicolon
  \caption{Atualização do Estado do Simulador}
  \label{alg:update}
 ...
 \Input{$G(V,E)$, $\lambda$}
  ...
  $I_s \leftarrow floor(Z_s)$ \;
  $I_c \leftarrow floor(Z_c)$ \;
  \BlankLine

  \tcp{Cria células se os valores forem positivos}
  \tcp{e remove caso contrário}
  $atualizarCelulas(I_s, I_c)$\;
  \BlankLine

  $Z_s \leftarrow Z_s - I_s$ \;
  $Z_c \leftarrow Z_c - I_c$ \;
  \BlankLine
